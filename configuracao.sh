...
URL_DISTRIBUICAO=http://<ip do servidor>
...
md5servidor (){
   scp -o StrictHostKeyChecking=no <usuario>@<ip do servidor>:/home/<servidor>/sumservidor.md5 .
   scp -o StrictHostKeyChecking=no <usuario>@<ip do servidor>:/home/<servidor>/arquivoserv.txt .
...
build () {
/bin/ping -w 1 -c 1 <qualquer host de teste>
if [ $? -eq 0 ];
then
rm build_num.info
...
if [ -f $HOME_<usuario>/Downloads/touch ]
then
echo "Touch existe" 
build
else
echo "$DATA touch ṇo existe" &>> /var/log/touch.log
/usr/bin/touch $HOME_<usuario>/Downloads/touch
/bin/chown <usuario>:<usuario> $HOME_<usuario>/Downloads/touch
echo "Data $DATA IP $IP hostname $NOME" | ssh -o StrictHostKeyChecking=no  <usuario>@$SERVIDOR_IP "cat - >> ips.txt"
fi